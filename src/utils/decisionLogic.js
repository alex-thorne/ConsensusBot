 ...
 * @returns {object} Vote counts by type
 */
...
const calculateSimpleMajority = (voteCounts) => {
  ...
  return {
    ...
    reason: passed 
      ...
      : `Simple majority not achieved. Need >50%, got ${percentage.toFixed(2)}%`,
    voteCounts,
    percentage
  };
};
 ...
 * 
 * @param {object} voteCounts - Vote counts object
 * @param {number} requiredVotersCount - Total number of required voters
 * @returns {object} Result object with status and details
 */
const calculateSupermajority = (voteCounts, requiredVotersCount) => {
 ...
 * @param {object} voteCounts - Vote counts object
 * @param {number} requiredVotersCount - Total number of required voters
 * @param {number} quorum - Minimum number of votes required (defaults to requiredVotersCount)
 * @returns {object} Result object with status and details
 */
const calculateUnanimity = (voteCounts, requiredVotersCount, quorum = null) => {
  const { yes, no, abstain, total } = voteCounts;
  ...
  return {
    passed: true,
    reason: `Unanimity achieved with ${yes} yes vote(s) and ${abstain} abstention(s)`,
    voteCounts,
    percentage,
    quorum: effectiveQuorum,
    quorumMet: true
 ...
 * 
 * @param {Array<object>} votes - Array of vote objects
 * @param {string} successCriteria - Success criteria (simple_majority, super_majority, unanimous)
 * @param {number} requiredVotersCount - Total number of required voters
 * @param {number|null} quorum - Optional quorum (for unanimity)
 * @returns {object} Decision outcome result
 */
const calculateDecisionOutcome = (votes, successCriteria, requiredVotersCount, quorum = null) => {
  logger.debug('Calculating decision outcome', {
 ...
 * @param {number} requiredVotersCount - Total number of required voters
 * @returns {object} Deadlock status
 */
const checkDeadlock = (votes, successCriteria, requiredVotersCount) => {
  const voteCounts = calculateVoteCounts(votes);
